const { pgTable, text, timestamp, uuid, integer, boolean, jsonb, decimal, varchar } = require('drizzle-orm/pg-core');
...
const imageAnalyses = pgTable('image_analyses', {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id').references(() => users.id).notNull(),
    patientId: uuid('patient_id').references(() => patients.id),
...
const riskPredictions = pgTable('risk_predictions', {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id').references(() => users.id).notNull(),
    patientId: uuid('patient_id').references(() => patients.id).notNull(),
