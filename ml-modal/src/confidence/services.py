...
def get_prediction_with_confidence(model, input_tensor):
    """
    Wrapper to get model prediction along with confidence, uncertainty,
    and entropy-based metrics.
    """
    mean_preds, variance_preds, predictive_entropy, mutual_information = get_mc_predictions(model, input_tensor)
    confidence_data = calculate_uncertainty(mean_preds, variance_preds, predictive_entropy, mutual_information)
